...
function trade_gallina(lena: number): number {
    return lena - 6
}

function trade_cabra(lena: number): number {
    return lena - 5
}

function trade_caballo(lena: number): number {
    return lena - 12
}

function trade_huevo(lena: number): number {
    return lena - 3
}

function trade_patata(lena: number): number {
    return lena - 2
}
...
controller.A.onEvent(ControllerButtonEvent.Pressed, function on_a_pressed() {
    ...
    if (cercaCofre == 1) {
        if (inventarioAbierto == 0) {
            cofreInventario.setImage(assets.image`
                cofreAbierto
                `)
            inventarioAbierto = 1
            controller.moveSprite(nena, 0, 0)
            menuInventario = miniMenu.createMenuFromArray([miniMenu.createMenuItem("Huevos: " + ("" + huevos), img`
                            . . 2 2 b b b b b . . . . . . .
                            . 2 b 4 4 4 4 4 4 b . . . . . .
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.inventario, function on_on_overlap(sprite: Sprite, otherSprite: Sprite) {
...
let trader = sprites.create(assets.image`
        comerciante
